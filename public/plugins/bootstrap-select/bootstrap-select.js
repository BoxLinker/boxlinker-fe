...
(function(root, factory) {
 ...
 // needed to support `apply`/`call` with `undefined`/`null`
  ...
  $.expr.pseudos.icontains = function (obj, index, meta) {
    let $obj = $(obj).find('a');
    let haystack = ($obj.data('tokens') || $obj.text()).toString().toUpperCase();
  ...
  $.expr.pseudos.aicontains = function (obj, index, meta) {
    let $obj = $(obj).find('a');
    let haystack = ($obj.data('tokens') || $obj.data('normalizedText') || $obj.text()).toString().toUpperCase();
    return haystack.includes(meta[3].toUpperCase());
  };
  ...
  let htmlUnescape = createEscaper(unescapeMap);
    ...
    constructor: Selectpicker,
      ...
      let generateA = function (text, classes, inline, tokens) {
        ...
        let $selectOptions = this.$element.find('option');
        ...
        let optionClass = this.className || '',
          ...
          let showDivider = this.previousElementSibling && this.previousElementSibling.tagName === 'OPTGROUP';
            ...
            prevHiddenIndex = $this.data('prevHiddenIndex');
              ...
              let prevHidden = $selectOptions.eq(prevHiddenIndex)[0].previousElementSibling;
    ...
    setStyle (style, status) {
      ...
      this.findLis();
        ...
        let that = this,
          ...
          selectOffsetRight,
          getPos = function() {
            let pos = that.$newElement.offset(),
     ...
     */
      ...
      let that = this,
        ...
        that.$lis.filter('.active').removeClass('active');
          ...
          if (that.$searchbox.val()) {
          let $searchBase = that.$lis.not('.is-hidden, .divider, .dropdown-header'),
              $hideItems;
